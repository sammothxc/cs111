...
def picky_piggy(score):
    ...
    if score == 0:
        ...
        # the inverse of the decimal
        decimal_expansion = 758241
        decimals = 6
        digit = (score - 1) % decimals
        points = decimal_expansion // 10 ** digit % 10
        return points
    # END PROBLEM 2
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        if who == 0:
            ...
            score = score1
        current_gain = score - last_score
        if current_gain > running_high:
            print(f"{current_gain} point(s)! That's a record gain for Player {who}!")
            return announce_highest(who, score, current_gain)
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averages(*args):
        ...
        for i in range(trials_count):
            averaged_num += original_function(*args)
        return averaged_num/trials_count
    return averages
    # END PROBLEM 8
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    best_outcome = 0
    for i in range(1, 11):
        average_rolls = make_averaged(roll_dice, trials_count)
        score = average_rolls(i, dice)
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    outcome = picky_piggy(opponent_score)
    if outcome >= cutoff:
        return 0
    else:
        return num_rolls
