...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            if isinstance(self, ContainerAnt) is True and isinstance(place.ant, ContainerAnt) is False:
                self.ant_contained = place.ant
                place.ant.remove_from(place)
                self.add_to(place)
                self.ant_contained.place = self.place

            elif isinstance(place.ant, ContainerAnt) is True and isinstance(self, ContainerAnt) is False and place.ant.ant_contained is None:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        current = self.place
        bees_lst = current.bees[:]
        if random_bee(current.bees) is not None:
            for b in range(len(bees_lst)):
                if self.health - amount <= 0:
                    bees_lst[b].reduce_health(self.damage + amount)
                else:
                    bees_lst[b].reduce_health(amount)
...
class ContainerAnt(Ant):
    ...
    def store_ant(self, ant):
        # BEGIN Problem 8
        if self.can_contain(ant) is True:
