...
class Ant(Insect):
    ...
    def buff(self):
        ...
        # BEGIN Problem 12
        if self.buffed:
             if self.is_container == True and self.ant_contained != None:
                if self.ant_contained.buffed == False:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place.is_hive!= True:
            if random_bee(place.bees):
                if count <= self.max_range and count >= self.min_range:
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
          for i in range(len(self.place.bees)-1,-1,-1):
            if Ant.buffed == True:
                self.place.bees[i].reduce_health((amount+self.damage)*2)
            else:    
                self.place.bees[i].reduce_health(amount+self.damage)  
        else:
...
class QueenAnt(ScubaThrower):
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.placed == True:
            return None
        else:
            gamestate.placed = True
