...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r"[IVX]{1,3}", text)
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'(?i)\bCS\s?\d+[A-C]?\b', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'(?:[01]\d|2[0-3]):[0-5]\d(?:AM|PM)?|\d:[0-5]\d(?:AM|PM)?', text)
...
def most_common_code(text):
    ...
    if not area_codes_list:
        return None
    else:
        return max(area_codes_list, key=area_codes_list.count)
